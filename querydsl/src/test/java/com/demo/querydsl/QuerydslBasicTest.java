...
import com.demo.querydsl.dto.MemberDto;
import com.demo.querydsl.dto.QMemberDto;
import com.demo.querydsl.dto.UserDto;
...
public class QuerydslBasicTest {
    ...
    public void searchAndParam() throws Exception {
        Member findMember = queryFactory
                ...
                .where(
                        ...
                        member.age.eq(10) // where절에 and를 많이 사용하기 때문에 .and() 체인을 되지만 이렇게 콤마로 나열해도 됩니다.
                )
                .fetchOne();
    ...
    /**
     * 예) 회원과 팀을 조인하면서, 팀 이름이 teamA인 팀만 조인하고, 회원은 모두 조회하시오.
    ...
    public void complexCase() {
        List<String> result = queryFactory
                .select(new CaseBuilder()
                        .when(member.age.between(0, 20)).then("0~20살입니다.")
                        .when(member.age.between(21, 30)).then("21~30살입니다.")
                        .otherwise("기타"))
                .from(member)
                .fetch();
    ...
    public void findDtoByJPQL() {
        List<MemberDto> result = em.createQuery("select new com.demo.querydsl.dto.MemberDto(m.username, m.age) from Member m", MemberDto.class)
                .getResultList();
    ...
    public void findDtoByField() {
        // 위 Projections.bean()은 Setter를 통해 값을 세팅하는 반면, Projections.fields()는 필드에 직접적으로 값을 세팅합니다.
        List<MemberDto> result = queryFactory
    ...
    public void findDtoByConstructor() {
        // 위 Projections.bean()과 Projections.fields()는 필드명과 조회하려는 엔티티 필드명이 동일해야 합니다.
        List<MemberDto> result = queryFactory
    ...
    public void findUserDto() {
        // ExpressionUtils로 감싸면 서브쿼리에 alias를 줄 수 있습니다.
        QMember memberSub = new QMember("memberSub");
